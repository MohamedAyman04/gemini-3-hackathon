...
window.addEventListener("message", (event) => {
  // Only trust our own messages
  if (event.data?.type === "VIBECHECK_AUTH_SUCCESS") {
    console.log(
      "VibeCheck (Isolated): Auth Success detected from Web Page",
      event.data.user,
    );
    if (extensionAPI) {
      extensionAPI.runtime
        ...
        .catch((err) => console.error("Failed to forward auth success:", err));
    }
  }
});
...
if (extensionAPI) {
  ...
  console.error(
    "VibeCheck (Isolated): CRITICAL - No Extension API found (chrome/browser undefined). Check execution context.",
  );
}
