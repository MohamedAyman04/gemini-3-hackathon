...
export const useAuth = () => {
    ...
    const checkAuth = async () => {
        ...
        try {
            ...
            const mockUser: User = {
                id: 'user_123',
                name: 'Test Engineer',
                email: 'tester@gemini.hackathon',
                avatarUrl: `https://ui-avatars.com/api/?name=Test+Engineer&background=random`
            };
    ...
    useEffect(() => {
        ...
        const handleCookieChange = (changeInfo: chrome.cookies.CookieChangeInfo) => {
            if (changeInfo.cookie.name === AUTH_COOKIE_NAME && changeInfo.cookie.domain.includes('localhost')) {
                checkAuth();
            }
        }
    ...
    const debugLogin = () => {
        setState({
            ...
            user: {
                ...
                avatarUrl: 'https://ui-avatars.com/api/?name=Debug+Dev&background=random'
            },
            error: null
        });
    };
